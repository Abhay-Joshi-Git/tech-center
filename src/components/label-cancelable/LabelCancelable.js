...
class LabelCancelable extends Component {
  ...
  render() {
    ...
    const closeBtnClassName = `${
      disabled ? "d-none" : "close cursor-pointer ml-2 float-right"
    }`;
    return (
      <span
        className={"label-wrapper d-inline-block pr-2 pl-2 mr-2 mb-2"}
      ...
      >
        ...
        <span className="label-text fs-12">{displayLabel}</span>
      </span>
    );
  }
}
